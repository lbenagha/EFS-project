resource "null_resource" "newlocal" {
  depends_on = [aws_efs_mount_target.mount_efs,aws_instance.web_ec2,]
  ...
  provisioner "remote-exec" {
    inline = ["sudo chmod ugo+rw /etc/fstab","sudo echo '${aws_efs_file_system.efs.id}:/ /var/www/html efs tls,_netdev' >> /etc/fstab","sudo mount -a -t efs,nfs4 defaults","sudo rm -rf /var/www/html*" ,"sudo git clone https://github.com/Vishnukvsvk/LW-TASK1.git /var/www/html"]
  }
}
